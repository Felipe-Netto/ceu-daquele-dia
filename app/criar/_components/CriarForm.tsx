...
export default function CriarForm({ preco, precoFormatado }: { preco: number; precoFormatado: string }) {
  ...
  const validate = (step: number): boolean => {
    ...
    if (step === 1) {
      ...
      else if (formState.data > new Date().toISOString().split('T')[0]) errs.data = 'A data não pode ser no futuro'
    }
    if (step === 2) {
                ...
                <div className="w-full overflow-hidden">
                  <input
                    ...
                    onChange={e => {
                      ...
                      const valor = e.target.value > hoje ? hoje : e.target.value
                      setFormState(prev => ({ ...prev, data: valor }))
